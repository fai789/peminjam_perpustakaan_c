      ...
      body: SingleChildScrollView( // Tambahkan SingleChildScrollView di sini
        child: Column(
          children: [
            ...
            SizedBox(height: 70),
            // Expanded widget diganti dengan Container
            Container(
              // Gunakan MediaQuery untuk mendapatkan tinggi layar
              height: MediaQuery.of(context).size.height - 196, // Kurangi dengan tinggi bagian atas dan bawah halaman
                      ...
                      child: ListTile(
                        ...
                        trailing: ElevatedButton(
                          onPressed: () => Get.toNamed(Routes.ADD_PEMINJAMAN,
                              parameters: {'id': (dataBook.id ?? 0).toString(), 'judul': dataBook.judul ?? '-', 'penulis': dataBook.penulis ?? " ",'sinopsis': dataBook.sinopsis ?? " ", 'penerbit': dataBook.penerbit ?? " ", 'tahun_terbit': (dataBook.tahunTerbit ?? 0).toString()}),
                          child: Text("Pinjam"),
                        ),
                      ),
                    );
